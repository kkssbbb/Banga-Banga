...
import { teamEvaluateService } from "../services";

const teamEvaluateRouter = Router();

teamEvaluateRouter.post("/", async (req, res, next) => {
  try {
    const { rater_id, matching_post_id, recritment_id, is_evaluate } = req.body;
    const userInfo = {
      user_name,
      mobile_number,
      email,
    ...
    const user = await teamEvaluateRouter.addUser(userInfo);
